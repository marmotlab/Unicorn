...
class ActorNet(nn.Module):
    def __init__(self, input_dim, agent_dim, int_vec_dim, hidden_dim=256, vae_hidden_dim=20):
        super().__init__()
        ...
        self.normalization = Normalization(self.flat_dim)
    ...
    def forward(self, state, phase_vector, int_vector, mask, h_n, num_meta):
        # State normalization
        # [b*n, flat_dim]
        state = state.reshape(-1, self.flat_dim)
        ...
        state_embedding = self.decoder(phase_vec_embedding, state_embedding, mask.reshape(-1, mask.size(-1)))
...
class CriticNet(nn.Module):
    def __init__(self, input_dim, agent_dim, int_vec_dim, hidden_dim=256, vae_hidden_dim=20):
        ...
        self.hidden_dim = hidden_dim
        self.vae_hidden_dim = vae_hidden_dim
        self.neighbor_action_dim = self.input_dim[0]
        ...
        self.linear_neighbor_a = nn.Sequential(
            nn.Linear(self.neighbor_action_dim, hidden_dim // 2),
            nn.ReLU(),
            nn.Linear(hidden_dim // 2, hidden_dim),
            nn.ReLU()
        )
        self.decoder_s = Decoder(embedding_dim=hidden_dim, n_head=4)
        self.decoder_a = Decoder(embedding_dim=hidden_dim, n_head=4)
        self.gru = nn.GRU(input_size=hidden_dim, hidden_size=hidden_dim, batch_first=False)
        self.gru_norm = Normalization(hidden_dim)

        # Add phase vec into the vae input
        self.linear_vae = nn.Sequential(
            nn.Linear(self.flat_dim + self.flat_int_vec_dim + self.flat_phase_vec_dim + self.neighbor_action_dim, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, hidden_dim),
            nn.ReLU()
        )
    ...
    def forward(self, state, phase_vector, int_vector, mask, h_n, neighbor_action_vector, num_meta):
        # State normalization
        # [b*n, d_s]
        state = state.reshape(-1, self.flat_dim)
        state = self.normalization(state)
        ...
        state = state.reshape(-1, self.agent_dim * num_meta, self.flat_dim)
        # [b, n, d_na]
        neighbor_action_vector = neighbor_action_vector.reshape(-1, self.agent_dim * num_meta, self.neighbor_action_dim)
        # [b, n, d_s] -> [b, n, d]
        state_embedding = self.linear_s(state)
        # [b, n, d_na] -> [b, n, d]
        neighbor_a_embedding = self.linear_neighbor_a(neighbor_action_vector)
        batch_dim = state_embedding.size(0)
        ...
        # Decoder for ego actions
        # [b*n, a_dim, d]
        phase_vector = phase_vector.reshape(batch_dim * self.agent_dim * num_meta, -1, self.flat_phase_vec_dim)
        phase_vec_embedding = self.linear_a(phase_vector)
        # [b*n, a_dim, d]
        state_embedding = state_embedding.reshape(batch_dim * self.agent_dim * num_meta, -1, self.hidden_dim)
        state_embedding = self.decoder_s(phase_vec_embedding, state_embedding, mask.reshape(-1, mask.size(-1)))

        # VAE
        # Add phase vector and neighbor actions vector into the vae input
        # [b*n, a_dim, d_s+d_i+d_p+d_na]
        int_vector = int_vector.reshape(batch_dim * self.agent_dim * num_meta, -1, self.flat_int_vec_dim)
        state_vector = state.reshape(batch_dim * self.agent_dim * num_meta, -1, self.flat_dim)
        state_vector = state_vector.expand(state_vector.size(0), int_vector.size(1), state_vector.size(2))
        neighbor_actions_vector = neighbor_action_vector.reshape(batch_dim * self.agent_dim * num_meta, -1, self.neighbor_action_dim)
        neighbor_actions_vector = neighbor_actions_vector.expand(neighbor_actions_vector.size(0), int_vector.size(1), neighbor_actions_vector.size(2))
        # [b*n, a_dim, d_s+d_i]
        # vae_input = torch.cat((state_vector, int_vector), -1)
        vae_input = torch.cat((state_vector, int_vector, phase_vector, neighbor_actions_vector), -1)
        vae_input = self.linear_vae(vae_input)
        mu = self.linear_mean(vae_input)
        logvar = self.linear_logvar(vae_input)
        std = torch.exp(0.5 * logvar)
        ...
        # Decode for neighbor action
        # [b*n, 1, d]
        neighbor_a_embedding = neighbor_a_embedding.reshape(-1, 1, self.hidden_dim)
        # [b*n, a_dim, d]
        state_embedding = self.decoder_a(state_embedding, neighbor_a_embedding)
...
class Unicorn(nn.Module):
    ...
    def forward_v(self, state, phase_vector, int_vector, mask, h_n, neighbor_actions, num_meta=1):
        return self.critic_network(state, phase_vector, int_vector, mask, h_n, neighbor_actions, num_meta)


if __name__ == '__main__':
    s = torch.randn((10, 5, 22, 5))
    phase_vec = torch.randn((10, 5, 6, 22))
    int_vec = torch.randn((10, 5, 6, 56))
    phase_mask = torch.zeros((10, 5, 6))
    phase_mask[0, 0, 3:] = torch.ones(3)
    neighbor_action = torch.zeros((10, 5, 22))

    model = Unicorn(input_dim=[22, 5], agent_dim=5, int_vec_dim=56, actor_lr=1e-5, critic_lr=1e-5)
    pi, _, pred1, mu1, logvar1 = model.forward(s, phase_vec, int_vec, phase_mask, None)
    v, _, pred2, mu2, logvar2 = model.forward_v(s, phase_vec, int_vec, phase_mask, None, neighbor_action)
