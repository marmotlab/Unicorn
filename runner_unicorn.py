...
class Runner(object):
    ...
    def __init__(self, meta_agent_id):
        ...
        self.neighbor_reward = TRAIN_PARAMS.NEIGHBOR_REWARD
        ...
        self.local_network = Unicorn(input_dim=self.env.tls_obs_space,
    ...
    def initialization(self):
        ...
        self.return_buffers = [[] for _ in range(10)]
        self.bootstrap_values = None
        self.rnn_states_actor = None
        self.rnn_states_critic = None
    ...
    def run_episode_single_threaded(self):
        ...
        while True:
            ...
            # Calculate the current neighbor's actions
            multi_agent_neighbor_actions_dict = self.env.get_neighbor_actions(action_dict)
            multi_agent_neighbor_actions = list(multi_agent_neighbor_actions_dict.values())
            multi_agent_neighbor_actions = convert_to_tensor(data=multi_agent_neighbor_actions,
                                                             data_type=torch.float32,
                                                             device=self.device)

            next_obs_n, r_n, done, info = self.env.step(action_dict)

            multi_agent_local_rewards = list(r_n.values())
            if self.neighbor_reward:
                raise NotImplementedError
            else:
                multi_agent_rewards = multi_agent_local_rewards
            ...
            self.experience_buffers[7].append(multi_agent_neighbor_actions)
    ...
    def calculate_advantage_values(self):
        ...
        batch_neighbor_actions_vec = torch.stack(self.experience_buffers[7])

        batch_reward = np.array(self.experience_buffers[2])

        with torch.no_grad():
            multi_agent_value, _, _, _, _ = self.local_network.forward_v(batch_obs, batch_phase_vec, batch_int_attr_vec,
                                                                         batch_phase_mask, None, batch_neighbor_actions_vec)
        ...
        self.return_buffers[7] = batch_neighbor_actions_vec[:-1]
        self.return_buffers[8] = advantage_values
        self.return_buffers[9] = target_values
