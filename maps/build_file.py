...
def output_net_config():
    ...
    str_config += '  </input>\n  <output>\n'
    str_config += '    <output-file value="grid_network_5_5.net.xml"/>\n'
    str_config += '  </output>\n</configuration>\n'
    return str_config
...
def gen_cfg_file(path, seed=None, thread=None):
    ...
    route_path = path + flow_file

    gen_arterial_rou_file(route_path=route_path,
                          seed=seed)
...
def gen_arterial_rou_file(route_path, seed):
    pass


def output_config(thread=None):
    if thread is None:
        out_file = 'grid_network_5_5.rou.xml'
    else:
        out_file = 'exp_%d.rou.xml' % int(thread)
    str_config = '<configuration>\n  <input>\n'
    str_config += '    <net-file value="grid_network_5_5.net.xml"/>\n'
    str_config += '    <route-files value="%s"/>\n' % out_file
    str_config += '    <additional-files value="grid_network_5_5.xml"/>\n'
    str_config += '  </input>\n  <time>\n'
...
def output_tls(tls, phase):
    ...
    five_phases = ['GGgrrrGGgrrr', 'yyyrrryyyrrr',
                   'rrrGrGrrrGrG', 'rrryryrrryry',
    ...
    for i in range(1, 26):
        node = 'nt' + str(i)
        str_adds += tls % node
        for k, p in enumerate(five_phases):
...
def main():
    ...
    # net config file
    write_file('./exp.netccfg', output_net_config())

    # generate net.xml file
    os.system('netconvert -c exp.netccfg')

    # raw.rou.xml file
    write_file('./grid_network_5_5.rou.xml', output_flows(1000, 2000, 0.2))

    # generate rou.xml file
    # os.system('jtrrouter -n grid_network_5_5.net.xml -r exp.raw.rou.xml -o grid_network_5_5.rou.xml')
    ...
    write_file('./grid_network_5_5.xml', output_ild(ild))

    # config file
    write_file('./grid_network_5_5.sumocfg', output_config())


if __name__ == '__main__':
    net_path = './grid_network_5_5.net.xml'
    route_path = './exp_0.rou.xml'
    seed = 12
    write_file(net_path, output_flows(1000, 2000, 0.2))
