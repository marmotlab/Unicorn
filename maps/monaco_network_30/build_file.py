...
def output_config(thread=None):
    if thread is None:
        out_file = 'monaco_network_30.rou.xml'
    else:
        out_file = 'monaco_network_30_{}.rou.xml'.format(int(thread))

    str_config = '<configuration>\n  <input>\n'
    str_config += '    <net-file value="monaco_network_30.net.xml"/>\n'
    str_config += '    <route-files value="{}"/>\n'.format(out_file)
    str_config += '    <additional-files value="monaco_network_30.add.xml"/>\n'
    str_config += '  </input>\n  <time>\n'
...
def gen_ma2c_monaco_sumocfg_file(path, flow_rate, seed=None, thread=None):
    if thread is None:
        flow_file = 'monaco_network_30.rou.xml'
        sumocfg_file = '/monaco_network_30.sumocfg'.format(thread)
    else:
        flow_file = '/monaco_network_30_{}.rou.xml'.format(int(thread))
        sumocfg_file = '/monaco_network_30_{}.sumocfg'.format(thread)
    write_file(path + flow_file, output_flows(flow_rate, seed=seed))

    sumocfg_file = path + sumocfg_file
    write_file(sumocfg_file, output_config(thread=thread))

    print('Generate Monaco sumocfg file: {}.'.format(sumocfg_file))
...
if __name__ == '__main__':
    ...
    # write_file('./real_net/data/in/monaco_network_30.add.xml', output_ild(env, ild))
    # env.terminate()
    gen_ma2c_monaco_sumocfg_file('/', 325, seed=0, thread=0)
